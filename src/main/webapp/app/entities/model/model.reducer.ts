...
export const ACTION_TYPES = {
  SEARCH_MODELS: 'model/SEARCH_MODELS',
...
export default (state: ModelState = initialState, action): ModelState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_MODELS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_MODELS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_MODELS):
...
const apiSearchUrl = 'api/_search/models';

// Actions

export const getSearchEntities: ICrudSearchAction<IModel> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_MODELS,
  payload: axios.get<IModel>(`${apiSearchUrl}?query=${query}`),
});
